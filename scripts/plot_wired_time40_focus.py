...
PLOT_DIR = Path("plots/wired_time40_focus")
...
def main():
    ...
    rtt = {}
    for file in DATA_DIR.glob("Tcp*-rtt.dat"):
        ...
        if times:
            rtt[variant] = [times, values]

    if queue:
        plot_lines(
            queue,
            "Queue Size vs Time (TcpWestwoodPlusNew vs TcpDualModifiedTcpWestwoodPlusNew)",
            "Time (s)",
            "Queue Size (packets)",
            PLOT_DIR / "queue_size_vs_time_focus.png",
        )

    if rtt:
        plot_lines(
            rtt,
            "RTT vs Time (TcpWestwoodPlusNew vs TcpDualModifiedTcpWestwoodPlusNew)",
            "Time (s)",
            "RTT (s)",
            PLOT_DIR / "rtt_vs_time_focus.png",
        )
